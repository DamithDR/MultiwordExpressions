...
with open('examples/mwe/en/data/mwe/spanish/spanish_annotation.txt', encoding='utf-8') as f:
    rows = f.readlines()
print(len(rows))
...
for row_num in range(0, len(rows)):
    ...
    if split[1].strip().lower() == "multiword expression":
        ...
        print(split[1].strip().lower())
        invalid_rows.append(row)
        invalid_rows_count += 1

print(f'invalid count {invalid_rows_count}')

with open('examples/mwe/en/data/mwe/spanish/invalid_rows.txt', 'w', encoding='utf-8') as f:
    rows = f.writelines(invalid_rows)

print("done")
with open('examples/mwe/en/data/mwe/spanish/spanish_annotation_MWEfiltered.txt', 'w', encoding='utf-8') as f:
    rows = f.writelines(filtered_rows)
f.close()

with open('examples/mwe/en/data/mwe/spanish/spanish_annotation_MWEfiltered.txt', encoding='utf-8') as f:
    rows = f.readlines()
...
for row_num in range(0, len(rows)):
    ...
    if split[2].strip().lower() == "metaphorical":
        if not metaphorical_names.__contains__(split[0].strip().lower()):
            metaphorical_names.add(split[0].strip().lower())
            filtered_rows.append(row)
    elif split[2].strip().lower() == "not metaphorical":
        if not not_metaphorical_names.__contains__(split[0].strip().lower()):
            not_metaphorical_names.add(split[0].strip().lower())
            filtered_rows.append(row)

with open('examples/mwe/en/data/mwe/spanish/arboles_de_espana.txt', 'r', encoding='utf-8') as f:
    rows = f.readlines()
for row in rows:
...
for sent in sentences:
    ...
    for gram_id in range(0, len(n_grams_list)):
        ...
        while tokens_id_num < len(iterative_list):
            ...
            if multiword_names.__contains__(expression.lower().replace(',', '').replace('‘', '').replace('’', '')):
...
for i in range(0, len(token_list[0])):
    ...
    if tag_lst1_out == 'B' or tag_lst2_out == 'B' or tag_lst3_out == 'B' or tag_lst4_out == 'B':
        final_tag_list.append('B')
    elif tag_lst1_out == 'I' or tag_lst2_out == 'I' or tag_lst3_out == 'I' or tag_lst4_out == 'I':
...
dataset['sentence_id'] = tok_num_lst[0][:165720]
dataset.sentence_id.astype(int)
dataset['words'] = token_list[0][:165720]
dataset['labels'] = final_tag_list

dataset.to_csv('examples/mwe/en/data/mwe/processed/spanish/spanish_train.tsv', sep='\t', index=False)
