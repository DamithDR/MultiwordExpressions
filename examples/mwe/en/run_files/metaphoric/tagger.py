...
with open('examples/mwe/en/data/metaphoric/spanish/spanish_annotation.txt', encoding='utf-8') as f:
    rows = f.readlines()
print(len(rows))
...
with open('examples/mwe/en/data/metaphoric/spanish/invalid_rows.txt', 'w', encoding='utf-8') as f:
    rows = f.writelines(invalid_rows)

print("done")
with open('examples/mwe/en/data/metaphoric/spanish/spanish_annotation_filtered.txt', 'w', encoding='utf-8') as f:
    rows = f.writelines(filtered_rows)
f.close()

with open('examples/mwe/en/data/metaphoric/spanish/spanish_annotation_filtered.txt', encoding='utf-8') as f:
    rows = f.readlines()
...
with open('examples/mwe/en/data/metaphoric/spanish/arboles_de_espana.txt', 'r', encoding='utf-8') as f:
    rows = f.readlines()
for row in rows:
...
for sent in sentences:
    ...
    for gram_id in range(0, len(n_grams_list)):
        ...
        while tokens_id_num < len(iterative_list):
            ...
            if metaphorical_names.__contains__(expression.lower().replace(',', '').replace('‘', '').replace('’', '')):
...
dataset['sentence_id'] = tok_num_lst[0][:165719]
dataset.sentence_id.astype(int)
dataset['words'] = token_list[0][:165719]
dataset['labels'] = final_tag_list

dataset.to_csv('examples/mwe/en/data/metaphoric/processed/spanish/spanish_train.tsv', sep='\t', index=False)
