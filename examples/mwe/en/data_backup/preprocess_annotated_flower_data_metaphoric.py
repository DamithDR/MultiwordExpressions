...
with open('annotated_flower_data_metaphoric_deduplicated.txt', encoding='utf-8') as f:
    rows = f.readlines()
print(len(rows))
...
with open('invalid_rows.txt', 'w', encoding='utf-8') as f:
    rows = f.writelines(invalid_rows)

print("done")
with open('metaphoric_annotated.txt', 'w', encoding='utf-8') as f:
    rows = f.writelines(filtered_rows)
f.close()

with open('metaphoric_annotated.txt', encoding='utf-8') as f:
    rows = f.readlines()
...
dataset.to_csv('metaphoric_flower_dataset_all_final.tsv', sep='\t', index=False)
